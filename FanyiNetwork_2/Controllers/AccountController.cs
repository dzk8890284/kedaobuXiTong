...
namespace FanyiNetwork.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            ViewBag.Title = "登录 - 凡易课导部管理系统";

            return View();
        }
        public IActionResult Register(string key)
        {
            ViewBag.Title = "注册 - 凡易课导部管理系统";
        ...
        public async Task<IActionResult> Register([FromBody]User model)
        {
            if (model != null)
            {
                ...
                using (var session = _documentStore.LightweightSession())
                {
                    ...
                    var nameExisted = session.Query<User>().SingleOrDefault(x => x.name == model.name);
                    if (nameExisted != null)
                    {
                        return BadRequest("该用户姓名已存在!请联系管理员!");
                    }
